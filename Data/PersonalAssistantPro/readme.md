# PersonalAssistantPro (个人助手 Pro)

> **基于 OpenHarmony 6.0 (API 20) 的企业级综合事务管理解决方案**

## 📖 项目简介 (Introduction)

**PersonalAssistantPro** 并不是一个简单的功能演示 Demo，而是一个严格遵循 **ArkTS 严格模式 (Strict Mode)** 开发的**企业级应用架构参考模板**。

本项目基于 **OpenHarmony API 20** 构建，旨在解决开发者在构建中大型应用时面临的工程化痛点。不同于简单的“Hello World”，本项目提供了一套完整的解决方案，涵盖了从**基础设施建设**（日志、调试）到**核心业务实现**（加密、持久化、后台服务）的全链路实践。

**核心价值：**

* **架构规范**: 采用标准 MVVM + Service 分层架构，实现高内聚低耦合。
* **工程完备**: 内置自主研发的**智能日志系统**与**真机调试框架**，大幅提升开发与排错效率。
* **技术深度**: 深入实践 RDB 事务、UserIAM **生物识别**、AES-256 隐私加密及后台代理提醒等高阶能力。

---


## 🏗️ 系统架构 (System Architecture)

本项目采用经典的 **MVVM (Model-View-ViewModel)** 分层架构，结合 **SOA (Service-Oriented Architecture)** 服务化设计思想，确保 UI 与业务逻辑的彻底解耦。

### 1. 架构全景图

![架构.png](tu/%E6%9E%B6%E6%9E%84.png)

### 2. 分层设计规范

* **UI Layer (Pages/View)**:
* **职责**: 仅负责 UI 渲染与用户交互事件捕获。
* **规范**: `MainPage` 作为唯一根容器管理路由栈 (`NavPathStack`)，不包含具体业务逻辑。


* **ViewModel Layer**:
* **职责**: 充当 View 与 Model 的适配器，维护 UI 状态（使用 `@Observed`/`@ObjectLink`）。
* **规范**: 禁止直接操作数据库，必须通过 Service 层获取数据。


* **Service Layer**:
* **职责**: 核心业务逻辑的聚集地，实现跨页面逻辑复用（如：数据导出、通知分发）。
* **规范**: 服务类通常设计为单例或静态工具类，保持无状态或全局唯一状态。


* **Model Layer**:
* **职责**: 负责数据的持久化、加密/解密及实体定义。
* **规范**: 所有数据库操作通过 `RdbHelper` 统一代理，确保事务安全。

## 🧩 核心特性 (Core Features)

本项目聚焦于商务办公场景，实现了以下四大核心业务模块：

### 1. 📅 智能日程管理

* **自定义视图**: 抛弃原生简单的日历控件，自主封装高性能的 `MonthView` (月视图) 与 `WeekView` (周视图)，支持手势滑动切换。
* **复杂查询**: 支持按日期范围 (`queryRange`) 筛选，同时集成 `Search` 组件，实现了对标题、备注的多字段**模糊搜索**。

### 2. 🔐 隐私通讯录

* **安全存储**: 联系人姓名、关系等字段明文存储，但**手机号 (Phone)** 等敏感字段在落库前自动加密。
* **权限管控**: 列表页仅展示脱敏数据（如 `138****0000`），点击查看详情时触发**生物识别**，只有验证通过才解密展示完整号码。

### 3. 🔔 后台代理提醒

* **准时触达**: 基于系统级 `ReminderAgent` 机制，即使应用被清理出后台，仍能准时唤起系统通知。
* **交互闭环**: 通知栏支持“关闭”或“延后 10 分钟”操作，且操作结果会自动同步回应用内的日程状态。

### 4. 📦 数据中台

* **多格式导出**: 支持将本地数据库全量导出为 **HTML 统计报表** (内联 CSS 样式) 或 **XML 备份文件**。
* **文件沙箱**: 演示了标准的文件 IO 操作与应用沙箱 (`/data/storage/el2/base/files`) 管理。

---

## 🛠️ 关键技术实现 (Technical Highlights)

本项目深入实践了 OpenHarmony API 20 的多项高阶能力，以下是核心技术方案的详细解析。

### 1. 💾 高级数据持久化 (RDB)

> **代码路径**: `entry/src/main/ets/common/database/RdbHelper.ts`

**本项目**封装了一个通用的 `RdbHelper` 类，解决了原生 `relationalStore` API 使用繁琐的问题：

* **事务管理**: 所有写操作（Insert/Update/Delete）均包裹在 Transaction 中，确保数据一致性。
* **动态查询**: 实现了 SQL `LIKE` 语法的动态拼接，支持 `Search` 组件对数据库的多字段模糊匹配。
* **类型安全**: 严格遵循 ArkTS 类型规范，对 Entity 与 RDB 字段进行了完整的映射处理。

### 2. 🛡️ 隐私安全体系 (Crypto + UserIAM)

> **代码路径**: `entry/src/main/ets/common/utils/AesCryptoUtils.ts`

为满足企业级安全要求，**本项目**设计了一套完整的隐私保护流程：

* **AES-256 加密流程**:
1. **密钥生成**: 使用 `cryptoFramework.createSymKeyGenerator('AES256')` 生成对称密钥。
2. **加密 (Encrypt)**: `TextEncoder` 转码 -> 生成随机 16位 IV -> `Cipher` 加密 -> 拼接 IV 与密文 -> Base64 编码。
3. **落库**: 将 Base64 字符串存入 RDB。


* **生物识别流程**:
* 调用 `userAuth.getAvailableStatus()` 检查设备是否具备**识别**能力（指纹/人脸）。
* 拉起系统统一认证弹窗 (`AuthWidgetMgr`)，用户验证通过后，才执行 `AesCryptoUtils.decrypt()` 解密逻辑。


### 3. 🏗️ 统一跳转服务 (JumpService)

> **代码路径**: `entry/src/main/ets/services/JumpService.ts`

为了解耦 UI 层与系统能力，**本项目**实现了 `JumpService` 单例：

* **外部应用拉起**: 构建包含 `entities: ['entity.system.browsable']` 的 `Want`，通过 `context.startAbility` 拉起系统浏览器打开 URL。
* **系统设置跳转**: 定向拉起 `com.huawei.hmos.settings`，引导用户开启通知或**生物识别**权限。
* **DeepLink 解析**: 预留了 `pa://` 协议的解析逻辑，为跨应用调用提供接口。
  好的，这是 **第三阶段** 的内容，包含 **工程化与基础设施** 以及 **目录结构**。


## ⚙️ 工程化与基础设施 (Infrastructure)

为了解决大型项目中“日志噪点多”和“业务逻辑难以脱离 UI 调试”的痛点，本项目内置了两套自主研发的基础设施。

### 1. 📝 智能日志系统 (Smart Logger)

> **代码路径**: `entry/src/main/ets/common/utils/Logger.ts`

摒弃原生的 `console.log`，封装了基于 `hilog` 的 `Logger` 类，实现了**模块化管理**与**动态噪声过滤**。

* **自动 Tag**: 所有日志自动携带 `PA_Pro_{ModuleName}` 前缀，便于在 Log 面板正则筛选。
* **黑/白名单模式**: 支持在 `Logger.ts` 中配置 `FILTER_MODE`：
* `BLOCK_LIST` (黑名单): 默认打印所有，屏蔽列表中的模块（适合开发初期）。
* `ALLOW_LIST` (白名单): 默认不打印，仅关注列表中的模块（适合专注调试）。


* **智能格式化**: 直接传入 Object/JSON，自动序列化输出，无需手动 `JSON.stringify`。

### 2. 🧪 真机集成测试框架 (Debug Runner)

> **代码路径**: `entry/src/main/ets/debug/`

由于 OpenHarmony 应用涉及大量原生能力（RDB、UserIAM），纯单元测试难以覆盖真实环境。**本项目**设计了一套 **DebugRunner**，允许在真机上直接运行后端逻辑测试用例。

#### 如何添加新的测试用例 (3步走)：

1. **新建 Case**: 在 `debug/` 目录下新建类继承 `DebugCase`。
```typescript
export class MyNewTest extends DebugCase {
  readonly name = 'My Feature Test';
  async run(): Promise<void> {
    this.logInfo('Start...');
    // 直接调用 Service/Model 层逻辑，支持 await
    const result = await MyService.doSomething();
    this.logInfo('Result:', result);
  }
}

```


2. **注册**: 在 `DebugRunner.ts` 的工厂方法中返回该实例。
3. **运行**: 进入 App **"我的" -> "调试入口"**，即可触发运行并在 Log 控制台查看结果。

---

## 📂 目录结构 (Project Structure)

本项目结构清晰，遵循 **Feature-First** 与 **Layer-First** 混合的组织方式：

```text
entry/src/main/ets
├── common                  # [公共基础层]
│   ├── constants           # 全局常量 (StyleConstants, CommonConstants)
│   ├── database            # 数据库核心 (RdbHelper, Table Definitions)
│   └── utils               # 核心工具链 (AesCrypto, Logger, DateUtils, Validation)
├── debug                   # [测试基础设施]
│   ├── DebugRunner.ets     # 测试运行器核心
│   └── *Case.ts            # 各类自动化测试用例 (Crypto, RDB, Performance)
├── entryability            # [应用入口] Ability 生命周期管理
├── model                   # [数据模型层] (Entity + DAO)
│   ├── ContactModel.ts     # 联系人数据操作
│   ├── EventModel.ets      # 日程数据操作
│   └── SettingModel.ets    # 设置项数据操作
├── pages                   # [路由容器层]
│   ├── MainPage.ets        # 全局唯一根容器 (Navigation)
│   ├── SplashPage.ets      # 启动页
│   └── [BusinessPages]     # 业务页面 (Mine, Detail, Edit...)
├── services                # [业务服务层] (解耦核心逻辑)
│   ├── BiometricService    # 生物识别服务
│   ├── DataExportService   # 数据导出/报表生成服务
│   ├── JumpService         # 路由跳转与 DeepLink 服务
│   └── NotificationService # 后台通知服务
├── view                    # [UI 组件库]
│   ├── calendar            # 自研日历组件 (MonthView, WeekView)
│   ├── component           # 通用组件 (TitleBar, Dialog)
│   └── form                # 表单组件 (DatePicker, TextInput)
└── viewmodel               # [视图模型层] 处理 UI 状态流转

```