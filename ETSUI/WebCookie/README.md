...
本篇Codelab使用的是在线网页，需添加网络权限：ohos.permission.INTERNET。
...
- 应用弹框
  ...
  若用户已通过“设置cookies”完成cookie写操作，点击应用首页的“获取cookies”按钮，则应用弹窗中会带有“info=测试cookie写入”的信息。此时点击“验证cookies”按钮跳转至cookie信息验证界面，界面加载完成后，也会弹出带有“info=测试cookie写入”信息的弹窗。若用户未进行写操作，则弹窗信中无对应信息。
...
![](figures/zh-cn_image_0000001364935274.png)
...
首次打开应用时，应用首页Web组件内呈现的是登录界面。用户完成登录操作后，会跳转至账号中心界面。在用户不点击“删除cookie”按钮的情况下，用户关闭并再次打开应用，首页仍会跳转至账号中心界面。Web组件会自动存储所加载界面的cookie信息，包括登录的cookie信息。用户可以通过点击“删除cookie”按钮，清除所有cookie信息。首页呈现效果如下图所示：

![](figures/1.png)

首页布局简单，由“Web组件”应用标题、“Web组件内”内部标题、加载的网页和底部一排按钮组成。分别对应两个Text组件、一个Web组件以及一个Row容器组件。Row容器组件内包含四个链接按钮，为LinkButton自定义组件。代码如下：
...
自定义组件LinkButton由Text组件和Divider分隔器组件组成。最后一个按钮没有分隔器，通过isNeedDivider标识符判断是否添加Divider分隔器组件。代码如下：
...
每个按钮被点击时，都是调用operationMethod函数。函数根据不同操作，执行不同的代码内容。包括cookie的读、写和删除操作，以及页面跳转操作。代码如下：
  ...
  switch (this.buttonType) {
    ...
    case CookieOperation.SET_COOKIE:
      try {
        ...
        showDialog($r('app.string.write_fail'));
      }
      break;
...
免登录验页是用于验证：当用户在应用首页已完成登录操作，在应用的其他页面使用Web组件，访问同网站需要登录授权的页面时，可免去多余的登录操作。一个应用中的所有Web组件共享一个WebCookie，因此一个应用中Web组件存储的cookie信息，也是可以共享的。界面呈现效果如下：

![](figures/10.png)

该页面布局同样简单，由“Web组件”导航标题、“Web组件内”内部标题、加载的网页组成。分别对应一个Navigator导航组件、一个Text组件和一个Web组件。Navigator导航组件类型设置为返回（NavigationType.Back），其中包含有返回图标和标题，水平排列展示。代码如下：
