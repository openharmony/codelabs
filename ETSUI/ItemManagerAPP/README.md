...
- **空状态友好引导**：物品列表为空时，展示「📦 + 引导文案」可视化页面，提示使用者添加物品。
...
### 4. 👤 个性化用户中心
- **信息自定义**：独立「用户中心」页面，支持昵称修改、头像上传/更换，打造专属用户标识。
- **偏好持久化**：利用Preferences存储用户资料信息，应用重启后仍保留个性化设置。
...
## 🚀 快速开始
### 环境准备
1. 安装 DevEco Studio 5.0+ 版本（[下载地址](https://developer.openharmony.cn/develop/deveco-studio)）。
...
### 4. 个性化设置
- 点击底部「用户中心」标签页，点击头像区域更换头像，编辑昵称输入框修改昵称。
