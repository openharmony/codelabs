...
CoinNote 是一款基于 HarmonyOS (ArkTS) 开发的轻量级、功能丰富的个人记账应用。它不仅支持基础的收支记录，还针对现代生活场景提供了 **AA制结算** 和 **报销管理** 等高级功能，帮助用户更精准地掌控个人财务状况。
...
- **本地存储**：使用 HarmonyOS `RelationalStore` (SQLite) 本地数据库存储。
...
│           │   ├── pages/          # UI 页面
│           │   │   ├── Index.ets   # 主页 (包含记账、明细、统计、我的)
│           │   │   └── login/      # 登录/密码设置页
│           │   └── entryability/   # 应用入口能力
│           └── resources/          # 静态资源 (图片、字符串、颜色)
...
1. **环境准备**：
   - 安装最新版 [DevEco Studio](https://developer.huawei.com/consumer/cn/deveco-studio/)。
   - 配置 HarmonyOS SDK (API 9 或更高版本)。
...
2. **处理 AA/报销**：
   - 进入 "明细" 页。
   - 找到带有 "AA待收" 或 "待报销" 标签的账单。
   - 点击标签，根据提示输入金额或确认。
   - 状态更新为 "AA已收" 或 "已报销"，金额自动更新。
