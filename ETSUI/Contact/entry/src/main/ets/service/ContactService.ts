...
import { BusinessError } from '@ohos.base'; // 确保导入
...
export class ContactService {
  ...
  static async addContact(c: LocalContact): Promise<string> {
    ...
    try {
      ...
      // 假设通过姓名和电话找到刚添加的联系人
      const newContact = allContacts.find(item =>
  ...
  static async getAllContacts(): Promise<LocalContact[]> {
    ...
    return systemContacts.map(item => {
  ...
  static async diagnose401Error(key: string, context: common.Context): Promise<void> {
    ...
    try {
      ...
      // 查找匹配的联系人
      const foundContacts = allContacts.filter(contact => contact.key === key);
      console.error('3. 找到匹配的联系人数量:', foundContacts.length);
      if (foundContacts.length > 0) {
        console.error('4. 找到的联系人详情:', foundContacts.map(c => ({
          key: c.key,
          id: c.id,
          name: c.name?.fullName
        })));
        // 检查是否有重复key
        const duplicateKeys = allContacts.filter(c => c.key === key);
        if (duplicateKeys.length > 1) {
          console.error('5. 警告：发现重复的key！');
        }
      } else {
        console.error('4. 未找到匹配的联系人');
        console.error('5. 系统所有key列表:', allContacts.map(c => ({
          key: c.key,
          key类型: typeof c.key,
          name: c.name?.fullName
        })));
  ...
  static async isContactExist(key: string): Promise<boolean> {
    try {
      ...
      // 精确匹配key（string类型）
      const found = contacts.find(item => item.key === key);
  ...
  static async testContactSystem(): Promise<void> {
    ...
    try {
      ...
      const updatedContact = updatedContacts.find(c => c.key === firstContact.key);
